...
# # Need to Review - END
    ...
    # bookmarks =Account.objects.filter(bookmark_questions=data).count()
            ...
            if data.is_protected:
                ...
                if getRecentAnswer and request.user.editPostTimeOfUser:
                    if getEditingTime >= timezone.now() - \
                        ...
                        if request.user.Refiner_Illuminator_TagPostCounter >= 50:
                            TagBadge.objects.get_or_create(
                                awarded_to_user=request.user,
                                badge_type="SILVER",
                                tag_name="Refiner",
                        ...
                        if request.user.Refiner_Illuminator_TagPostCounter >= 500:
                            TagBadge.objects.get_or_create(
                                awarded_to_user=request.user,
                                badge_type="GOLD",
...
# Question Views
    ...
    if request.user.is_authenticated:
        created = Question.viewers.through.objects.get_or_create(
            question=datas, account_id=request.user.id)
...
# Show New Contributer Message - END
    ...
    if request.user.is_authenticated:
        if request.user.accessTo_moderatorTools:
            if request.method != 'POST':
...
def bookmarkIt(request, question_id):
    ...
    bookmarks =Account.objects.filter(bookmark_questions=post).count()
    print(bookmarks)
