...
from Teacher.Student_views.Grade_9_std.Grd9_ClsA_std.grd9_ClsA_forms_update_marks import (Grade_9_student_ClsA_updatemarks_english, Grade_9_student_ClsA_updatemarks_math, Grade_9_student_ClsA_updatemarks_science,
					Grade_9_student_ClsA_updatemarks_SocialScience, Grade_9_student_ClsA_updatemarks_PersonalDevelopment)
from django.contrib.postgres.search import SearchVector, SearchQuery
from Teacher.Teacher_views.Grade9_views.Grd9_teacher_models import Grade_9_teacher_classA, Grade_9_Teacher_Class

from Student.student_views.Grade9_std_views.Grade9_models import Grade_9_student_classA, Grade_9_Student, Term_11_math_assesment # Grade_9_Student
...
from Teacher.Teacher_views.Grade10_views.Grd10_assesment_models import (Grade_10_models_math_assesment,Grade_10_models_LanguageLiterature_assesment,
Grade_10_models_Science_assesment,
Grade_10_models_SocialScience_assesment,
Grade_10_models_Geography_assesment,
Grade_10_models_Chemistry_assesment,
Grade_10_models_PersonalDev_assesment)
from Teacher.Teacher_views.Grade9_views.Grd9_assesment_models import (
Grade_10_models_PersonalDev_assesment_Form,
Grade_10_models_math_assesment_Form,
Grade_10_models_SocialScience_assesment_Form,
Grade_10_models_Science_assesment_Form,
Grade_10_models_LanguageLiterature_assesment_Form,
Grade_10_models_Geography_assesment_Form,
Grade_10_models__Chemistry_assesment_Form
	)

#this handles models of upload forms
from Teacher.Teacher_views.Grd10_subj_upload_forms import (Grd10_class_Math_upload_form_1 ,Grd10_class_Math_upload_form_2 ,Grd10_class_English_upload_form_1
	,Grd10_class_English_upload_form_2, Grd10_class_Science_upload_form_1 ,Grd10_class_Science_upload_form_2
		,Grd10_class_SocialScience_upload_form_1 ,Grd10_class_SocialScience_upload_form_2 ,Grd10_class_PersonalDev_upload_form_1 ,Grd10_class_PersonalDev_upload_form_2)
#news models
from Teacher.Teacher_views.Grade10_views.Grd10_ClsA_views.Grd10ClsA_news_MVT import GrdClsA_news
'''
...
def class_teacher_checker(request,context):
	if Grade_10_Teacher_Class.objects.filter(id=request.user.id,is_Grade_10_teacher_classA=True):
		context['teacher_name_cls']='Teacher Class A'
		context['students_class']='List of Student Class A'
		context['students'] = Grade_10_Student.objects.filter(is_Grade_10_student_classA=True)
	elif Grade_10_Teacher_Class.objects.filter(id=request.user.id,is_Grade_10_teacher_classB=True):
		context['teacher_name_cls']='Teacher Class B'
		context['students'] = Grade_10_Student.objects.filter(is_Grade_10_student_classB=True)
		context['students_class']='List of Student Class B'
	else:
		print("nope")

def class_teacher_subj_checker(request,context):
	if Grade_10_Teacher_Class.objects.filter(id=request.user.id,Grade_10_Math_subj_teach=True):
		context['Subject'] = 'Math Teacher'
		context['Subj'] = 'Math'
		
	elif Grade_10_Teacher_Class.objects.filter(id=request.user.id,Grade_10_Engl_subj_teach=True):
		context['Subject'] = 'English Teacher'
		context['Subj'] = 'English'
		
	elif Grade_10_Teacher_Class.objects.filter(id=request.user.id,Grade_10_Scn_subj_teach=True):
		context['Subject'] = 'Science Teacher'
		context['Subj'] = 'Science'
		
	elif Grade_10_Teacher_Class.objects.filter(id=request.user.id,Grade_10_SocSnc_subj_teach=True):
		context['Subject'] = 'Social Science Teacher'
		context['Subj'] = 'Social Science'
		
	elif Grade_10_Teacher_Class.objects.filter(id=request.user.id,Grade_10_PersDev_subj_teach=True):
		context['Subject'] = 'Perosnal Development Teacher'
		context['Subj'] = 'Perosnal Development'
		
	else:
		context['Subj'] = 'Not assisgned'
def Grade_10_Math_subj_view(request):
	return render(request,'Grade10Teach/Grade10_Student_subj_templates/Grade10_subj_templates')

def View_Uploaded_assesment(request):
	class_teacher=Grade_10_Teacher_Class.objects.filter(id=request.user.id)
...
def View_Uploaded_assesment_Term1(request, tm): 
    ...
    class_teacher=Grade_10_Teacher_Class.objects.filter(id=request.user.id)
    assessments_list = []
    if Grade_10_Teacher_Class.objects.filter(id=request.user.id, Grade_9_Math_subj_teach=True):
	    if tm=="1":
	    	unique_assessments = Grade_10_models_math_assesment.objects.filter(Term1_assesment=True).values('Title').distinct()
	    	if Grade_10_models_math_assesment.objects.filter(Term1_assesment=True).values('Title').distinct().count()>=1:
			    for assessment in unique_assessments:
			        # Retrieve the latest assessment object for each unique title and term
			        latest_assessment = Grade_10_models_math_assesment.objects.filter(
			            Term1_assesment=True,
			            Title=assessment['Title']
			        ).order_by('-DatePosted').first()
			        assessments_list.append(latest_assessment)
	    if tm=="2":
	    	unique_assessments = Grade_10_models_math_assesment.objects.filter(Term2_assesment=True).values('Title').distinct()
	    	if Grade_10_models_math_assesment.objects.filter(Term2_assesment=True).count()>=1:
			    for assessment in unique_assessments:
			        # Retrieve the latest assessment object for each unique title and term
			        latest_assessment = Grade_10_models_math_assesment.objects.filter(
			        ...
			        assessments_list.append(latest_assessment)
	    if tm=="3":
	    	unique_assessments = Grade_10_models_math_assesment.objects.filter(Term3_assesment=True).values('Title').distinct()
	    	for assessment in unique_assessments:
		        # Retrieve the latest assessment object for each unique title and term
		        latest_assessment = Grade_10_models_math_assesment.objects.filter(
		        ...
		        assessments_list.append(latest_assessment)
	    if tm=="4":
	    	unique_assessments = Grade_10_models_math_assesment.objects.filter(Term4_assesment=True).values('Title').distinct()
	    	for assessment in unique_assessments:
		        # Retrieve the latest assessment object for each unique title and term
		        latest_assessment = Grade_10_models_math_assesment.objects.filter(
		        ...
		        assessments_list.append(latest_assessment)
    if Grade_10_Teacher_Class.objects.filter(id=request.user.id, Grade_9_Engl_subj_teach=True):
	    if tm=="1":
	    	unique_assessments = Grade_10_models_LanguageLiterature_assesment.objects.filter(Term1_assesment=True).values('Title').distinct()
	    	if Grade_10_models_LanguageLiterature_assesment.objects.filter(Term1_assesment=True).values('Title').distinct().count()>=1:
			    for assessment in unique_assessments:
			        # Retrieve the latest assessment object for each unique title and term
			        latest_assessment = Grade_10_models_LanguageLiterature_assesment.objects.filter(
			            Term1_assesment=True,
			            Title=assessment['Title']
			        ).order_by('-DatePosted').first()
			        assessments_list.append(latest_assessment)
	    if tm=="2":
	    	unique_assessments = Grade_10_models_LanguageLiterature_assesment.objects.filter(Term2_assesment=True).values('Title').distinct()
	    	if Grade_10_models_LanguageLiterature_assesment.objects.filter(Term2_assesment=True).count()>=1:
			    for assessment in unique_assessments:
			        # Retrieve the latest assessment object for each unique title and term
			        latest_assessment = Grade_10_models_LanguageLiterature_assesment.objects.filter(
			            Term2_assesment=True,
			        ...
			        assessments_list.append(latest_assessment)
	    if tm=="3":
	    	unique_assessments = Grade_10_models_LanguageLiterature_assesment.objects.filter(Term3_assesment=True).values('Title').distinct()
	    	for assessment in unique_assessments:
		        # Retrieve the latest assessment object for each unique title and term
		        latest_assessment = Grade_10_models_LanguageLiterature_assesment.objects.filter(
		            Term3_assesment=True,
		        ...
		        assessments_list.append(latest_assessment)
	    if tm=="4":
	    	unique_assessments = Grade_10_models_LanguageLiterature_assesment.objects.filter(Term4_assesment=True).values('Maths_Title').distinct()
	    	for assessment in unique_assessments:
		        # Retrieve the latest assessment object for each unique title and term
		        latest_assessment = Grade_10_models_LanguageLiterature_assesment.objects.filter(
		            Term4_assesment=True,
		        ...
		        assessments_list.append(latest_assessment)
    if Grade_10_Teacher_Class.objects.filter(id=request.user.id, Grade_9_Scn_subj_teach=True):
	    if tm=="1":
	    	unique_assessments = Grade_10_models_Science_assesment.objects.filter(Term1_assesment=True).values('Title').distinct()
	    	if Grade_9_models_Science_assesment.objects.filter(Term1_assesment=True).values('Title').distinct().count()>=1:
			    for assessment in unique_assessments:
			        # Retrieve the latest assessment object for each unique title and term
			        latest_assessment = Grade_10_models_Science_assesment.objects.filter(
			            Term1_assesment=True,
			            Title=assessment['Title']
			        ).order_by('-DatePosted').first()
			        assessments_list.append(latest_assessment)
	    if tm=="2":
	    	unique_assessments = Grade_10_models_Science_assesment.objects.filter(Term2_assesment=True).values('Title').distinct()
	    	if Grade_10_models_Science_assesment.objects.filter(Term2_assesment=True).count()>=1:
			    for assessment in unique_assessments:
			        # Retrieve the latest assessment object for each unique title and term
			        latest_assessment = Grade_10_models_Science_assesment.objects.filter(
			            Term2_assesment=True,
			        ...
			        assessments_list.append(latest_assessment)
	    if tm=="3":
	    	unique_assessments = Grade_10_models_Science_assesment.objects.filter(Term3_assesment=True).values('Title').distinct()
	    	for assessment in unique_assessments:
		        # Retrieve the latest assessment object for each unique title and term
		        latest_assessment = Grade_10_models_Science_assesment.objects.filter(
		            Term3_assesment=True,
		        ...
		        assessments_list.append(latest_assessment)
	    if tm=="4":
	    	unique_assessments = Grade_10_models_Science_assesment.objects.filter(Term4_assesment=True).values('Title').distinct()
	    	for assessment in unique_assessments:
		        # Retrieve the latest assessment object for each unique title and term
		        latest_assessment = Grade_10_models_Science_assesment.objects.filter(
		            Term4_assesment=True,
		        ...
		        assessments_list.append(latest_assessment)
    if Grade_10_Teacher_Class.objects.filter(id=request.user.id, Grade_9_SocSnc_subj_teach=True):
	    if tm=="1":
	    	unique_assessments = Grade_10_models_LanguageLiterature_assesment.objects.filter(Term1_assesment=True).values('Title').distinct()
	    	if Grade_10_models_LanguageLiterature_assesment.objects.filter(Term1_assesment=True).values('Title').distinct().count()>=1:
			    for assessment in unique_assessments:
	    ...
	    if tm=="3":
	    	...
	    	for assessment in unique_assessments:
		        ...
		        assessments_list.append(latest_assessment)
	    if tm=="4":
	    	unique_assessments = Grade_10_models_LanguageLiterature_assesment.objects.filter(Term4_assesment=True).values('Title').distinct()
	    	for assessment in unique_assessments:
		        # Retrieve the latest assessment object for each unique title and term
		        latest_assessment = Grade_10_models_LanguageLiterature_assesment.objects.filter(
		            Term4_assesment=True,
		        ...
		        assessments_list.append(latest_assessment)
    if Grade_10_Teacher_Class.objects.filter(id=request.user.id, Grade_9_PersDev_subj_teach=True):
	    if tm=="1":
	    	unique_assessments = Grade_10_models_LanguageLiterature_assesment.objects.filter(Term1_assesment=True).values('PersonalDev_Title').distinct()
	    	if Grade_9_models_LanguageLiterature_assesment.objects.filter(Term1_assesment=True).values('PersonalDev_Title').distinct().count()>=1:
			    for assessment in unique_assessments:
			        ...
			        assessments_list.append(latest_assessment) 
	    if tm=="2":
	    	unique_assessments = Grade_10_models_LanguageLiterature_assesment.objects.filter(Term2_assesment=True).values('PersonalDev_Title').distinct()
	    	if Grade_10_models_LanguageLiterature_assesment.objects.filter(Term2_assesment=True).count()>=1:
			    for assessment in unique_assessments:
			        # Retrieve the latest assessment object for each unique title and term
			        latest_assessment = Grade_10_models_LanguageLiterature_assesment.objects.filter(
			        ...
			        assessments_list.append(latest_assessment)
	    if tm=="3":
	    	unique_assessments = Grade_10_models_LanguageLiterature_assesment.objects.filter(Term3_assesment=True).values('PersonalDev_Title').distinct()
	    	for assessment in unique_assessments:
		        # Retrieve the latest assessment object for each unique title and term
		        latest_assessment = Grade_10_models_LanguageLiterature_assesment.objects.filter(
		            Term3_assesment=True,
		        ...
		        assessments_list.append(latest_assessment)
	    if tm=="4":
	    	unique_assessments = Grade_10_models_LanguageLiterature_assesment.objects.filter(Term4_assesment=True).values('PersonalDev_Title').distinct()
	    	for assessment in unique_assessments:
		        # Retrieve the latest assessment object for each unique title and term
		        latest_assessment = Grade_10_models_LanguageLiterature_assesment.objects.filter(
		            Term4_assesment=True,
    ...
    class_teacher_subj_checker(request,context)
    class_teacher_checker(request,context)
    return render(request, 'Grade10Teach/Grade10_Student_subj_templates/View_Uploaded_assesment_Term1.html', context)


def Grade_10_subj_view(request,pk,tm):
	class_teacher=Grade_10_Teacher_Class.objects.filter(id=request.user.id)

	context={'teachers':class_teacher}
	class_teacher_subj_checker(request,context)
	class_teacher_checker(request,context)
	return render(request,'Grade10Teach/Grade10_Student_subj_templates/Grade_10_all_subj_view.html',context)
...
def View_Uploaded_assesment_all_subj(request,snme):
	...
	if Grade_9_Teacher_Class.objects.filter(id=request.user.id,Grade_9_Math_subj_teach=True):
		...
		context['Subj'] = 'Math'
		sbj=Grade_10_models_math_assesment.objects.filter(Title=snme)
		context['sbj']=sbj
	elif Grade_10_Teacher_Class.objects.filter(id=request.user.id,Grade_9_Engl_subj_teach=True):
		context['subjects '] = 'English Teacher'
		context['Subj'] = 'English'
		sbj=Grade_10_models_LanguageLiterature_assesment.objects.filter(Title=snme)
		context['sbj']=sbj
	elif Grade_10_Teacher_Class.objects.filter(id=request.user.id,Grade_9_Scn_subj_teach=True):
		context['subjects '] = 'Science Teacher'
		context['Subj'] = 'Science'
		sbj=Grade_10_models_Science_assesment.objects.filter(Title=snme)
		context['sbj']=sbj
	elif Grade_10_Teacher_Class.objects.filter(id=request.user.id,Grade_9_SocSnc_subj_teach=True):
		context['subjects '] = 'Social Science Teacher'
		context['Subj'] = 'Social Science'
		sbj=Grade_10_models_SocialScience_assesment.objects.filter(Title=snme)
		context['sbj']=sbj
	elif Grade_10_Teacher_Class.objects.filter(id=request.user.id,Grade_9_PersDev_subj_teach=True):
		context['subjects '] = 'Perosnal Development Teacher'
		context['Subj'] = 'Perosnal Development'
		sbj=Grade_10_models_PersonalDev_assesment.objects.filter(Title=snme)
		context['sbj']=sbj
	else:
		context['Subj'] = 'Not assisgned'
	class_teacher_checker(request,context)
	class_teacher_subj_checker(request,context)
	return render(request, 'Grade10Teach/Grade10_Student_subj_templates/View_Uploaded_assesment_all_subj.html',context)
...
def Grade9_classA_teacher(request):
	...
	context={'teachers':class_teacher, 'Headmin_news':Headmin_news, 'teacher_news':teacher_news}
	class_teacher_checker(request,context)
	class_teacher_subj_checker(request,context)
	return render(request, 'Grade9Teach/Tgrd9A/TeacherGrd9ClsA.html', context) 
...
def student_details_Grd9_clsA(request, pk):
	...
	context={'students':all_students, 'teachers':class_teacher}
	class_teacher_checker(request,context)
	class_teacher_subj_checker(request,context)
